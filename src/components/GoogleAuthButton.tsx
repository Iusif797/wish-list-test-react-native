...
const OAUTH_CALLBACK_URL = extra.oauthCallbackUrl || "https://wish-list-dun.vercel.app/auth/callback";
  ...
  const handlePress = async () => {
    ...
    try {
      ...
      const urlRes = await fetch(
        getApiUrl(`/auth/oauth/google?redirect_uri=${encodeURIComponent(redirectUri)}&state=mobile`)
      );
      if (!urlRes.ok) {
      ...
      const res = await api<{ access_token: string; user: { id: string; email: string; name: string | null } }>(
        "/auth/oauth/google",
        {
          method: "POST",
