...
import { View, StyleSheet, Text, KeyboardAvoidingView, Platform, ScrollView, TouchableOpacity, SafeAreaView, Keyboard } from "react-native";
...
export default function EditItemScreen({ route, navigation }: Props) {
  ...
  const { data: wishlist } = useSWR<any>(`/wishlists/${id}`, fetcher);
  ...
  useEffect(() => {
    if (wishlist && !initialized) {
      const item = wishlist.items.find((i: any) => i.id === itemId);
      if (item) {
        setUrl(item.url);
        setName(item.name);
        setPrice(item.price.toString());
        if (item.target_amount) setTargetAmount(item.target_amount.toString());
        if (item.image_url) setImageUrl(item.image_url);
        setInitialized(true);
      }
    }
  }, [wishlist, itemId, initialized]);
    ...
    return (
      <SafeAreaView style={[styles.container, isDark ? styles.bgDark : styles.bgLight]}>
        <View style={styles.center}>
          <Text style={isDark ? styles.textDark : styles.textLight}>Загрузка...</Text>
        </View>
      </SafeAreaView>
    );
  }
      ...
      >
        <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.header}>
            <Text style={[styles.title, isDark ? styles.textDark : styles.textLight]}>Изменить подарок</Text>
          </View>
