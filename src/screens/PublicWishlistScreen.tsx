...
import { View, StyleSheet, Text, FlatList, TouchableOpacity, SafeAreaView, Alert, Modal, KeyboardAvoidingView, Platform } from "react-native";
...
export default function PublicWishlistScreen({ route, navigation }: Props) {
  ...
  const { data: wishlist, mutate, isLoading, error } = useSWR<any>(`/public/wishlists/${slug}`, fetcher);
  ...
  return (
    <SafeAreaView style={[styles.container, isDark ? styles.bgDark : styles.bgLight]}>
      ...
      {/* Contribute Modal */}
      <Modal visible={!!contributeItem} transparent animationType="slide" onRequestClose={() => setContributeItem(null)}>
        <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.modalOverlay}>
          <GlassCard style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={[styles.modalTitle, isDark ? styles.textDark : styles.textLight]}>Поддержать</Text>
            ...
            {contributeItem && (
              <View style={styles.modalItemInfo}>
                <Text style={[styles.modalItemName, isDark ? styles.textDark : styles.textLight]}>{contributeItem.name}</Text>
                <Text style={isDark ? styles.textMutedDark : styles.textMutedLight}>
                  Осталось собрать: {(contributeItem.target_amount ?? contributeItem.price) - contributeItem.total_contributed} ₽
                </Text>
              </View>
            )}
